...
class NsMapInt : public NsObj<theMax>
{
	typedef struct
	{
		uint32  nEntry;
...
public:
	...
	inline const TKey* GetKey(uint32 nIdx) { return this->GetBit(nIdx%theMax)?&mstData[nIdx%theMax].kKey:NULL; };
...
protected:
	...
	inline uint32 Hash(TKey kKey) { return IABS(kKey)%mnBckt; };
	inline bool IsEqual(TKey kKey1, TKey kKey2) { return bool(kKey1==kKey2); };
