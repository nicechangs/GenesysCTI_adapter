...
{
    CLogEx::INF("[PROCESS]");
    CLogEx::INF("%-20s : %s", "PROCESS_NAME"        , m_szProcName);
    CLogEx::INF("%-20s : %s", "PROCESS_ID"          , m_szProcId);
    CLogEx::INF("\r\n");

    CLogEx::INF("[SYSTEM]");
    CLogEx::INF("%-20s : %s", "SYSTEM_TYPE"         , m_szSystemType);
    CLogEx::INF("%-20s : %s", "SYSTEM_ID"           , m_szSystemId);
    CLogEx::INF("\r\n");

    CLogEx::INF("[CBA_INFO]");
    CLogEx::INF("%-20s : %s", "CB_LISTEN_IP"        , m_szCBListenIp);
    CLogEx::INF("%-20s : %d", "CB_LISTEN_PORT"      , m_nCBListenPort);
    ...
    // driver info    
    CLogEx::INF("[IVR_DRIVER]");
    CLogEx::INF("%-20s : %s", "REMOTE_IP"           , m_szDrvRemoteIp);
    CLogEx::INF("%-20s : %d", "REMOTE_PORT"         , m_nDrvRemotePort);    
    CLogEx::INF("%-20s : %s", "SVC_OBJECT_NAME"     , m_szDrvSvcObjectName);
    CLogEx::INF("%-20s : %s", "SVC_APP_NAME"        , m_szDrvSvcAppName);
    CLogEx::INF("%-20s : %d", "KEEP_ALIVE_INTERVAL" , m_nKeepAliveInterval);

    CLogEx::INF("%-20s : %s", "TE_SYSTEM_ID"        , m_szDrvTESystemId);
    CLogEx::INF("%-20s : %d", "START_CHANNEL"       , m_nDrvStartChannel);
    CLogEx::INF("%-20s : %d", "START_PORT"          , m_nDrvStartPort);
    CLogEx::INF("%-20s : %d", "DNIS_COUNT"          , m_nDrvDNISCount);
    CLogEx::INF("%-20s : %d", "LOG_LEVEL"           , m_nDrvLogLevel);
    CLogEx::INF("%-20s : %s", "LOG_PATH"            , m_szDrvLogPath);
    CLogEx::INF("%-20s : %s", "LOG_LINK_PATH"       , m_szDrvLogLinkPath);
    CLogEx::INF("\r\n");
