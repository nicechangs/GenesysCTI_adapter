...
NPVOID CCBSessionManager::ThreadRun(NPVOID pArg)
{
    ...
    while(IsOn())
    {
        ...
        QUEUE_ITEM qItem = CQueueOfResponse::Inst()->Dequeue();
        ...
        LOG_WRITE(ILOG_INFO, "%s:%d|%s <<< %s|Data=(%d)%s \r\n"
                            , pSession->GetIp()
                            , pSession->GetPort()
                            , pSession->GetName()
                            , PROC_NAME
...
bool CCBSessionManager::RemoveSession(NPCSTR szSessionName)
{
    ...
    {
        LOG_WRITE(ILOG_WARN,"%-20s|Remove CBSession fail. name:%s"
                            , GetName()
                            , szSessionName);
        return false;
    }

    LOG_WRITE(ILOG_INFO, "%-20s|Remove CBSession success. name:%s"
                            , GetName()
                            , szSessionName);
    return true;
}
